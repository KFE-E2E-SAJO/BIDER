...
export default function ChatRoom({ roomId }: { roomId: string }) {
  ...
  const messages = (roomData as any)?.chatRoomsDetail?.[0]?.messages || [];
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col bg-white">
      <div className="flex flex-col gap-2 border-b border-t border-neutral-100 px-4 pb-2 pt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-semibold">
              {(roomData as any)?.products?.[0]?.profiles?.nickname || '사용자'}
            </span>
          </div>
          <button>
            <svg
        ...
        <div className="mt-1 flex items-center gap-3 text-xs">
          <img
            src={(roomData as any)?.productImages?.[0]?.image_url || '/default-product.png'}
            alt="Product Image"
            className="h-9 w-9 rounded-md object-cover"
          />
          <div className="flex min-w-0 flex-col justify-center">
            <div className="truncate text-[13px] font-medium">
              {(roomData as any)?.products?.[0].title}
            </div>
            <div className="text-[15px] font-bold">
              {((roomData as any)?.auctions?.[0].min_price || 0).toLocaleString()}원
            </div>
          </div>
      ...
      <div className="flex flex-col items-center gap-1 py-4">
        <img
          src={(roomData as any)?.products?.[0]?.profiles?.profile_img || '/default-profile.png'} // 곰돌이 이미지
                ...
                group.messages.map((message: any, index: number) => {
                  return (
                    <Message
                      ...
                      avatar={message?.sender_profile_img}
