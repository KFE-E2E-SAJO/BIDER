...
export async function POST(request: Request, { params }: { params: { shortId: string } }) {
  ...
  try {
    ...
    // STEP 4: pending_auction 테이블 업데이트
    ...
    // STEP 5: 이미지 처리 로직
    // 5-1: 기존 이미지 중 유지할 것들과 삭제할 것들 구분 (기존 이미지 ID만 체크)
    const existingImageIds = existingImages?.map((img) => img.image_url) || [];
    ...
    const imagesToDelete = existingImages.filter(
      (img) => !orderExistingImageIds.includes(img.image_url)
    );
    ...
    for (const orderInfo of imageOrders) {
      if (orderInfo.id.startsWith('NEW_IMAGE_')) {
        ...
        const { error: updateOrderError } = await supabase
          .from('product_image')
          .update({ order_index: orderInfo.index })
          .eq('product_id', productId)
          .eq('image_url', orderInfo.id);

        if (updateOrderError) {
          throw new Error(`이미지 순서 업데이트 실패: ${updateOrderError.message}`);
        }
      }
    }
